...
/// <summary>
/// ONNX-backed low-light enhancer using Zero-DCE++ (Li et al., TPAMI 2021).
/// ~10 K parameters, ~52 KB ONNX. Takes an RGB image in [0,1] and predicts
/// pixel-wise tonal curves that lift the dark regions without amplifying
/// noise. Excellent for under-exposed snapshots, indoor/evening shots.
///
/// Input  shape: [1, 3, H, W] float32 in [0, 1]. Dynamic H/W.
/// Output shape: [1, 3, H, W] enhanced image in [0, 1].
///
/// Returns null when the model isn't installed — same degrade-gracefully
/// pattern as the other ONNX wrappers.
/// </summary>
public sealed class OnnxLowLightEnhancer : IDisposable {
  public const string DefaultModelFileName = "zero-dce-pp.onnx";

  private readonly Lazy<InferenceSession?> _session;

  public OnnxLowLightEnhancer(FileInfo? modelFile = null) {
  ...
  /// <summary>Enhance and return a freshly allocated image. Source is not mutated. Returns null when no model is available.</summary>
  public Image<Rgba32>? Enhance(Image<Rgba32> source, CancellationToken ct = default) {
  ...
  public async Task<Image<Rgba32>?> EnhanceAsync(Image<Rgba32> source, CancellationToken ct = default) {
    ArgumentNullException.ThrowIfNull(source);
    var session = this._session.Value;
    if (session == null)
      return null;
